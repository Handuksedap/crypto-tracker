  ...
  try {
    ...
    body.innerHTML = `
      <div class="modal-header">
        ...
        <div>
          <h2>${d.name} ${isMeme({ id, symbol: d.symbol, name: d.name }) ? '🐸' : ''}</h2>
          <div class="sym">${d.symbol} • Rank #${d.market_cap_rank || '—'}</div>
        </div>
      </div>
      ...
      <div class="modal-grid">
        ...
        <div class="modal-item"><div class="lbl">Circulating Supply</div><div class="val">${md.circulating_supply?.toLocaleString('en-US', { maximumFractionDigits: 0 }) || '—'}</div></div>
        <div class="modal-item"><div class="lbl">ATH Date</div><div class="val">${md.ath?.usd ? new Date(md.ath_date?.usd).toLocaleDateString('id-ID') : '—'}</div></div>
      </div>
      ${desc ? `<div class="modal-desc">${desc}${desc.length === 400 ? '…' : ''}</div>` : ''}
      ${d.links?.homepage?.[0] ? `<div class="modal-desc"><a href="${d.links.homepage[0]}" target="_blank" rel="noopener">${d.links.homepage[0]} →</a></div>` : ''}
    `;
  } catch (e) {
